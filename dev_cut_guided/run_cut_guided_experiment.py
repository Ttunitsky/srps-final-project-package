...
from dev_cut_guided.cut_guided_ops import build_cut_state, make_cut_guided_destroy, make_cut_guided_repair
...
OUT_DIR = os.path.join(ROOT, "results", "cut_guided_dev")
...
def _parse_args():
    ...
    p.add_argument(
        ...
        default=True,
        help="Enable/disable the cut-guided destroy operator",
    )
    p.add_argument(
        "--cut-repair",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Enable/disable the cut-guided repair operator",
    )
    p.add_argument(
        "--cut-weight",
        type=float,
        default=0.35,
        help="Penalty weight for cut pressure in repair scoring",
    )
...
def _run_one(args_tuple):
    ...
    ub = meta["ub"]
    cut_state = build_cut_state(best_sol)
    use_cut_destroy = cfg["cut_destroy"]
    use_cut_repair = cfg["cut_repair"]
    cut_weight = cfg["cut_weight"]

    def _cut_provider():
        return cut_state

    cut_destroy = make_cut_guided_destroy(_cut_provider)
    cut_repair = make_cut_guided_repair(_cut_provider, cut_weight=cut_weight)
    ...
    if use_cut_destroy:
        destroy_ops.insert(0, cut_destroy)
    if use_cut_repair:
        repair_ops.insert(0, cut_repair)
    ...
    return {
        ...
        "cut_feasible": int(bool(cut_state.get("feasible", False))),
        "cut_pressure": round(float(cut_state.get("pressure", 0.0)), 6),
        "cut_critical_path_len": int(cut_state.get("critical_path_len", 0)),
        "phases": phase,
        "lag_calls": lag_calls,
        "lag_iters": lag_iters,
        "cut_destroy": int(use_cut_destroy),
        "cut_repair": int(use_cut_repair),
...
def main():
    ...
    cfg = {
        "cut_destroy": bool(args.cut_destroy),
        "cut_repair": bool(args.cut_repair),
    ...
    out_csv = os.path.join(OUT_DIR, f"cut_guided_{args.tag}_{ts}.csv")

    print("=" * 72)
    print("Cut-guided dev run (isolated)")
    print(f"Instances: {len(tasks)} | Workers: {args.workers}")
    print(
        "Switches: "
        f"cut_destroy={cfg['cut_destroy']} "
        f"cut_repair={cfg['cut_repair']} "
        f"cut_weight={cfg['cut_weight']}"
    )
    ...
    fields = [
        ...
        "ub", "final_gap_pct", "delta_rt_s", "delta_gap_pct", "cut_feasible", "cut_pressure", "cut_critical_path_len",
        "phases", "lag_calls", "lag_iters",
        "cut_destroy", "cut_repair",
