...
cut-augmented arm achieves a mean certified gap of $0.428\%$ against $0.533\%$
for the plain Lagrangian arm (12 instance wins to 8; exploratory, as both arms
share incumbents --- $ub$ differences are the clean per-instance evidence).
This is the one dual-side technique in the study that tightens the bound, and its
benefit is conditional on the initial upper bound having genuine headroom.
...
the Lagrangian modifies the effective profit for job $j$ on processor $k$'s subproblem
to $\mu_{j,k} + \gamma_k b_j / O_k$.  Each per-processor subproblem remains a
standard orienteering DP on the same arc graph, with augmented edge weights.
...
\end{proposition}
\begin{proof}
The $\gamma_k$ penalties enter linearly through
$\mu_{j,k}\leftarrow\mu_{j,k}+\gamma_k b_j/O_k$, so each $O_k(\mu,\gamma)$ is
still a standard orienteering DP.  The incompatibility penalties shift the
$y$-threshold before the selection rule fires, and do not enter the route
subproblems.
\end{proof}
...
with deliberately weakened initial bounds the cut-augmented arm wins 12 of 54
instance comparisons against 8 for plain LR (exploratory; both arms share
