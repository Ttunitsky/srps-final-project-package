...
Problem with Synchronisation: a Falsification Study\\
of Strengthening Techniques and a Cut-Augmented\\
...
We also develop a cut-augmented Lagrangian relaxation $L(\mu,\gamma,\nu)$ that
inserts capacity and incompatibility inequalities in the projected selection
space while preserving the per-processor orienteering decomposition. In
standalone comparison on a 54-instance stratified sample, this tightens the
certified bound against a matched-iteration plain Lagrangian baseline, winning on
30 of 54 instances with a mean gap of $0.361\%$ versus $0.455\%$ ($-0.094\pp$).
In a pipeline experiment with deliberately weakened initial bounds, the
cut-augmented arm achieves a mean certified gap of $0.451\%$ against $0.559\%$
for the plain Lagrangian arm on 52 valid instances (12 instance wins to 8;
exploratory, as both arms share incumbents --- $ub$ differences are the clean
per-instance evidence; 2 instances excluded where the weakened bound was
inadvertently invalid).  This is the one dual-side technique in the study that
tightens the bound, and its benefit is conditional on the initial upper bound
having genuine headroom.
...
the per-processor subproblems is $1.42\times$ faster with numerically identical bounds (floored certificates match exactly).
...
improves what it targets. Beyond the ten-technique study, a cut-augmented
Lagrangian extension (Section~\ref{sec:cutlr}) is the one dual-side contribution
that tightens the bound. Sections~\ref{sec:dual}--\ref{sec:runtime} are organised
...
\begin{enumerate}
  ...
  \item A cut-augmented Lagrangian relaxation $L(\mu,\gamma,\nu)$
    (Section~\ref{sec:cutlr}) incorporating capacity and incompatibility
    inequalities in the projected selection space while preserving the
    per-processor decomposition; evaluated across three experimental protocols
    (standalone budget comparison, tight-bound pipeline, headroom pipeline)
    establishing when and why it tightens the certified bound.
\end{enumerate}
...
\input{fragments/coverage_results_cutlr}
...
% ──────────────────────────────────────────────────────────────────────────────
\section{Cut-augmented Lagrangian relaxation}\label{sec:cutlr}

Two cut-separation routes are closed by the later falsification study
(Section~\ref{sec:cuts}): the primal incumbent is feasible by construction
and can never supply a violated cut, and the natural feasibility cut against
the Lagrangian relaxed solution is unsound on most instances.  An alternative
route avoids both pitfalls by incorporating valid inequalities directly into the
\emph{dual} as Lagrangian penalties, so the bound-computing mechanism itself
becomes tighter.  This section develops and evaluates that route, independently
of the falsification study.

\subsection{Formulation}\label{sec:cutlr-formulation}

The Lagrangian relaxation $L(\mu)$ decomposes per-processor (Section~\ref{sec:lagbound}).
The cut-augmented extension $L(\mu,\gamma,\nu)$ adds two families of valid inequalities
in the projected $y$-space, while preserving this decomposition.

\subsubsection{Capacity cuts}

For each processor $k$, let $O_k$ denote the single-processor orienteering optimum at
zero multipliers: the maximum profit processor $k$ can earn independently.  Any
feasible SRPS solution selects only jobs that processor $k$ can accommodate within the
horizon $L$, so the total profit of selected jobs requiring $k$ is at most $O_k$:
\begin{equation}\label{eq:cap-cut}
  \sum_{j \in J_k} b_j\, y_j \;\le\; O_k \qquad \forall\, k \in K.
\end{equation}
Introducing multipliers $\gamma_k \ge 0$ for these constraints and folding them into
the Lagrangian subtracts $\sum_{k:\,j\in J_k}\gamma_k b_j/O_k$ from job $j$'s
selection threshold; the per-processor orienteering subproblems retain profits
$\mu_{j,k}$ unchanged.  Each $O_k(\mu,\gamma)$ is therefore a standard orienteering
DP on the same arc graph.

\subsubsection{Incompatibility cuts}

A pair $(i,j)$ is \emph{incompatible} if no single-processor route can accommodate
both jobs simultaneously (verified by attempting to include both in any per-processor
DP).  For incompatible pairs:
\begin{equation}\label{eq:pair-cut}
  y_i + y_j \;\le\; 1.
\end{equation}
A triple $(i,j,\ell)$ is \emph{incompatible} if no route can accommodate all three:
\begin{equation}\label{eq:triple-cut}
  y_i + y_j + y_\ell \;\le\; 2.
\end{equation}
Multipliers $\nu_{ij}\ge 0$ and $\nu_{ij\ell}\ge 0$ penalise violations, adding a
per-job offset to the selection threshold before each per-processor solve.

\begin{proposition}[Validity]\label{prop:cuts-valid}
Constraints~\eqref{eq:cap-cut}--\eqref{eq:triple-cut} are valid for SRPS.
\end{proposition}
\begin{proof}
Any feasible SRPS solution selects only jobs every required processor can serve
within $L$, so the selected profit requiring processor $k$ is at most $O_k$,
giving~\eqref{eq:cap-cut}. An incompatible pair cannot both appear in any
feasible route, so~\eqref{eq:pair-cut} holds by definition;
similarly~\eqref{eq:triple-cut}.
\end{proof}

\begin{proposition}[Decomposition preserved]\label{prop:decomp}
$L(\mu,\gamma,\nu)$ separates into the same per-processor orienteering subproblems
as $L(\mu)$, with shifted job profits.
\end{proposition}
\begin{proof}
Both $\gamma_k$ and $\nu$ penalties enter only the $y$-selection threshold:
$y_j=1$ iff $\mathrm{profit}_j - \sum_k\mu_{j,k}
- \sum_{k:\,j\in J_k}\gamma_k b_j/O_k - \nu\text{-terms} > 0$.
The per-processor route subproblems maximise $\sum_{j\in J_k}\mu_{j,k}\,x_{j,k}$
subject to orienteering constraints, with profits $\mu_{j,k}$ unchanged by
$\gamma$ or $\nu$.  Each $O_k(\mu,\gamma,\nu)$ is therefore a standard
orienteering DP.
\end{proof}

Three configurations are evaluated: \textbf{cap} (capacity cuts only),
\textbf{pair} (capacity and pair incompatibilities), and
\textbf{triple} (all three).  All multipliers are updated by projected
subgradient steps alongside $\mu_{j,k}$, warm-started between invocations.

\subsection{Experimental protocols}\label{sec:cutlr-design}

Three protocols probe where the cuts add value.

\paragraph{Standalone comparison (stratified-54).}
Each method receives the same total dual budget: 100 cold-start Lagrangian
iterations, optionally followed by 100 warm cut-augmented iterations.
The baseline $\text{LR\_200\_cold}$ doubles the cold budget; the control
$\text{LR\_100{+}LR\_100}$ applies the same 200 iterations as a warm
continuation without cuts.  All comparisons are gap-only: no ALNS primal
is run.

\paragraph{Tight-UB pipeline (hard-6).}
Six hard instances run through the full adaptive pipeline
(Section~\ref{sec:loop}), with the initial $ub$ set to the master-result
upper bound from the production run --- the tightest certificate already
available.  This probes whether an in-loop dual refresh can improve on it.

\paragraph{Headroom pipeline (stratified-54).}
The same 54-instance set runs through the full pipeline, with the initial
$ub$ deliberately set to $\lfloor L_{\text{200,cold}}\rfloor$ per instance ---
the weakened value from the standalone analysis.  This gives the in-loop dual
refresh genuine headroom to tighten.  Two arms are compared: plain LR in-loop
versus cutLR-triple in-loop.

\subsection{Standalone results}\label{sec:cutlr-standalone}

\input{fragments/cutlr_standalone_results}

\subsection{Pipeline results}\label{sec:cutlr-pipeline}

\subsubsection{Tight-UB protocol: the ceiling applies}

With the master upper bound as the initial $ub$, the in-loop dual refresh fires
but cannot tighten the certificate: both the LR arm and the cutLR-triple arm
record \textbf{zero successful refreshes} across all six hard instances.  The
certified gaps are identical between arms.

This is the ceiling of Section~\ref{sec:ceiling} operating directly.  The
initial $ub$ was produced with a 200-iteration, 60-second dual budget; the
in-loop refresh has only 100 iterations and 30 seconds.  A tighter input cannot
be improved by a smaller budget, and $\lfloor L_{\text{in-loop}}\rfloor\ge ub_0$
on every instance.

\subsubsection{Headroom protocol}

\input{fragments/cutlr_headroom_results}

\subsection{Verdict}\label{sec:cutlr-verdict}

The cut-augmented Lagrangian tightens the bound in standalone
comparison.  With the same number of iterations, cutLR-triple reduces mean certified gap
from $0.455\%$ to $0.361\%$ and wins on 30 of 54 instances; the plain warm
continuation (LR\_100$+$LR\_100) achieves only $0.467\%$ on the same budget,
confirming the improvement is from the cuts rather than the warm start alone.

The pipeline result is conditional.  With a tight initial $ub$ the ceiling
absorbs both arms and neither gains.  Under the headroom protocol, cutLR-triple
achieves mean $0.428\%$ against $0.533\%$ for plain LR, winning 12 instance
comparisons to LR's 8.  The advantage is concentrated on medium-hard instances
($\alpha=0.5$, $n\ge 80$), where per-processor capacity conflicts make
subgradient LR slow to converge.  On the hardest $\alpha=0.25$ instances,
accumulated warm-multiplier LR calls eventually converge past what cutLR
achieves per call within the 30-second cap, giving LR a countervailing edge on
8 instances.

Two caveats apply to the headroom experiment.  First, the deliberate weakening
of the initial $ub$ does not reflect the production pipeline, where the initial
certificate comes from a prior full run; the headroom protocol is a
stress-test, not a simulation.  Second, the two pipeline arms share incumbents:
ARM 1 (LR) updated the canonical incumbent directory, so ARM 2 (cutLR) started
from those improved solutions on some instances.  The $ub$ comparison is
unaffected by this contamination, but the gap figures on instances where ARM 1's
ALNS regressed should be read as approximate; the UB delta is the clean
...
\midrule
\multicolumn{2}{l}{\emph{Cut-augmented Lagrangian (Section~\ref{sec:cutlr})}} \\
$O_k$ & single-processor orienteering optimum for processor $k$ at zero multipliers \\
$\gamma_k$ & multiplier on capacity cut for processor $k$; $\gamma_k\ge 0$ \\
$\nu_{ij}$ & multiplier on pair incompatibility $y_i+y_j\le 1$; $\nu_{ij}\ge 0$ \\
$\nu_{ij\ell}$ & multiplier on triple incompatibility $y_i+y_j+y_\ell\le 2$; $\nu_{ij\ell}\ge 0$ \\
\bottomrule
\end{tabular}
\end{table}
...
We described a certified primal--dual solver for SRPS, tried ten times over to
strengthen it with techniques from the exact-algorithms syllabus (nine of which
failed on structural grounds), and then exhibited a positive dual-side
contribution: a cut-augmented Lagrangian relaxation that tightens
the bound when the initial certificate has genuine headroom.

\textbf{Falsification study: nine of ten attempts fail.}

\textbf{Cut separation fails three times over} (Section~\ref{sec:cuts}). Separation against a feasible
incumbent is
...
what it targets: $1.42\times$ on the dual with numerically identical bounds (floored certificates match exactly), capturing
...
\textbf{The cut-augmented Lagrangian is the one positive dual-side result}
(Section~\ref{sec:cutlr}).  Capacity and incompatibility inequalities in the
projected $y$-space, combined via Lagrangian penalties while preserving the
per-processor decomposition, tighten the certified bound on 30 of 54 stratified
instances in standalone comparison ($0.361\%$ mean versus $0.455\%$ for a
matched-iteration plain LR baseline, $-0.094\pp$).  In a pipeline experiment
with deliberately weakened initial bounds the cut-augmented arm wins 12 of 52
valid instance comparisons against 8 for plain LR (exploratory; both arms share
incumbents --- certified $ub$ differences are the clean per-instance evidence),
with advantage concentrated on medium-hard instances.  The
benefit is conditional: where the initial certificate is already tight the
ceiling absorbs both arms equally, and on the hardest low-alpha instances
accumulated warm-multiplier subgradient calls eventually outpace per-call
cutLR.  The net picture is that cuts add real value at the start of a solve or
after a change in instance data, but not inside a re-solve whose initial bound
is already the output of a full production run.
